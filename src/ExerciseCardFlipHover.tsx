...
import './ExerciseCardFlipHover.css';

interface ExerciseCardFlipHoverProps {
  title: string;
  subtitle: string;
  exercises: string[];
...
const ExerciseCardFlipHover: React.FC<ExerciseCardFlipHoverProps> = ({
  title,
  subtitle,
  exercises,
  backSubtitle,
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    // Resetear completamente el transform y aplicar solo el flip
    if (innerRef.current) {
      // Forzar el reset del transform antes de aplicar el nuevo
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (isFlipped || !containerRef.current || !innerRef.current) return;
    ...
    rafIdRef.current = requestAnimationFrame(() => {
      if (!containerRef.current || !innerRef.current || isFlipped) return;
      ...
      // Aplicar solo el efecto 3D del mouse (el flip se maneja con CSS)
  ...
  return (
    <div 
      ref={containerRef}
      className={`flip-card-hover ${className} ${isFlipped ? 'flipped' : ''}`}
    ...
    >
      <div 
        ...
        style={{
          transition: isHovered && !isFlipped ? 'none' : 'transform 0.6s ease-in-out',
        }}
      >
        {/* Cara frontal */}
        <div className="flip-card-front">
          ...
          {/* Indicador de flip - emoji 🔄, aparece en hover, en la parte inferior */}
          {hasBackContent && (
...
export default ExerciseCardFlipHover;
